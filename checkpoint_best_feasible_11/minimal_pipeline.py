...
def _loss_db(distance_m: float, freq_mhz: float, system_loss: float, obstruction: bool) -> float:
    fspl = 32.45 + 20 * math.log10(freq_mhz) + 20 * math.log10(max(distance_m / 1000, 1e-9))
    return fspl + system_loss + (obstruction * 10.0)


def _segment_cert(a: dict, b: dict, h_a: float, h_b: float, dem: dict, threshold_db: float,
                  link: tuple[float, float, float, float]) -> dict:
    ...
    loss = _loss_db(d3, 2400.0, 3.0, obstructed)
    return {"distance_m": d3, "min_clearance_m": min_clear, "unknown": unknown,
            "obstructed": obstructed, "loss_db": loss, "feasible": (not unknown and min_clear > 0 and loss <= threshold_db)}
...
def q3(data: dict, tasks: pd.DataFrame, threshold_shift_db: float = 0.0) -> pd.DataFrame:
    ...
    for _, t in tasks.iterrows():
        ...
        direct = _segment_cert(p, s, gateway_h, cruise_h, dem, direct_thr, (20.0, 3.0, 12.0, 0.0))
        best = None
            ...
            for frac in (0.25, 0.5, 0.75):
                ...
                for offset in (100.0, 200.0, 300.0):
                    ...
                    for ut in route_ts:
                        ...
                        access_certs.append(_segment_cert(up, rp, cruise_h, rh, dem, access_thr, (20.0, 3.0, 6.0, 0.0)))
                    ...
                    b = _segment_cert(rp, p, rh, gateway_h, dem, back_thr, (19.0, 8.0, 12.0, 0.0))
