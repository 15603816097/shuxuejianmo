...
def test_c1_c2_c3_shared_core():
    d=fixture(); assert q3_c1_final.evaluate(d,('S008',),'B',['S008-MED-01'])['hard_deadline_ok'] is True; assert q3_c2_final.evaluate(d,('S005','S008'),'C',['S008-MED-01','S005-MED-01'])['timeline']['makespan_s']>=0; assert q3_c3_final.evaluate(d,('S005','S009','S008'),'C',['S008-MED-01','S005-MED-01','S009-MED-01'])['timeline']['makespan_s']>=0
def test_deadline_mutation_is_hard():
    d=fixture(); r=evaluate_route(d,('S008',),'B',['S008-MED-01']); r['deadline_ledger'][0]['pass']=False; assert not r['deadline_ledger'][0]['pass']
def test_ordinary_soft_does_not_change_hard():
    d=fixture(); r=evaluate_route(d,('S008',),'B',['S008-FOD-01']); assert r['hard_deadline_ok'] is True
def test_payload_changes_after_handoff():
    x=build_authoritative_timeline(fixture(),('S005','S008'),'C',['S008-MED-01','S005-MED-01']); assert x['segments'][1]['payload_before']<x['segments'][0]['payload_before']
def test_return_reserve_gate():
    d=fixture(); r=evaluate_route(d,('S008',),'B',['S008-MED-01'],transport_reserve_ok=False); assert r['TRANSPORT_FEASIBLE'] is False
def test_threshold_mutation_changes_communication_gate():
    d=fixture(); r=evaluate_route(d,('S008',),'B',['S008-MED-01'],communication_ok=False); assert r['ROUTE_FEASIBLE'] is False
def test_component_overlap_mutation():
    d=fixture(); c1=component_ledger(d,'C01',0,100,.1); c2=component_ledger(d,'C01',50,150,.1,100); assert c2['resource_available_ok'] is False
def test_no_fallback_status():
    assert status(False,False,False,False)['ROUTE_FEASIBLE'] is False
