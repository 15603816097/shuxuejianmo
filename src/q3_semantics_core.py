...
def link_obstructed(data, a, b, ha, hb):
    """Return whether the 30 m DEM contains a LOS obstruction.

    The terrain profile is already split at DEM cell boundaries.  The LOS
    height is linear in the same parameter, so a non-positive clearance at
    any returned cell/boundary record is a conservative obstruction test.
    """
    profile = terrain_profile(a, b, data['dem'])
    for t, ground in profile:
        if np.isfinite(ground):
            los_h = float(ha) + float(t) * (float(hb) - float(ha))
            if los_h - float(ground) <= 0.0:
                return True
    return False
...
def build_authoritative_timeline(data, visit_order, aircraft_type, box_ids):
    ...
    for nxt in tuple(visit_order)+('O01',):
        segment_payload_before=payload
        ...
        segments.append({'from':prev,'to':nxt,'phases':leg,'energy_kwh':energy,'payload_before':segment_payload_before,'payload_after':payload})
        prev=nxt
...
def direct_and_relay_blocks(data,timeline):
    ...
    for p in timeline['events']:
        ...
        dmax=max(np.linalg.norm(xyz(data,p['a'],p['ha'])-G),np.linalg.norm(xyz(data,p['b'],p['hb'])-G)); direct_obs=obs if link_obstructed(data,p['a'],data['center'],p['ha'],float(c['海拔（m）'])+float(data['comm'][('固定网关 G01','天线离地高度（m）')])) or link_obstructed(data,p['b'],data['center'],p['hb'],float(c['海拔（m）'])+float(data['comm'][('固定网关 G01','天线离地高度（m）')])) else 0.0; margin=td-(32.45+20*math.log10(freq)+20*math.log10(max(dmax/1000,1e-12))+direct_obs)
...
def continuous_interval_certificate(data, segment, endpoint, endpoint_h):
    ...
    clearance=min(clear) if clear else float('nan'); applied_obs=obs if np.isfinite(clearance) and clearance<=0 else 0.0; path=fspl+applied_obs; return {'distance_max':dmax,'distance_max_method':'endpoints+quadratic_stationary','fspl_upper_bound':fspl,'path_loss_upper_bound':path,'threshold':threshold,'margin_lower_bound':threshold-path,'los_clearance_lower_bound':clearance,'obstacle_loss_applied_db':applied_obs,'continuous_certified':bool(np.isfinite(clearance) and clearance>0 and threshold-path>=0),'dem_cells':len(cells)}
...
def evaluate_route(data, visit_order, aircraft_type, box_ids, transport_reserve_ok=True,
                   communication_ok=False, relay_ok=False, component_ok=False,
                   resource_ok=True):
    """Single final gate used by C1/C2/C3 wrappers; no post-hoc fallback."""
    tl=build_authoritative_timeline(data,visit_order,aircraft_type,box_ids); dl=deadline_ledger(data,tl); en=transport_energy_ledger(data,tl)
    medical_ok=all(x['pass'] for x in dl if x['source']=='MEDICAL_HARD'); first_ok=all(x['pass'] for x in dl if x['source']=='FIRST_BATCH_HARD'); hard_ok=medical_ok and first_ok
    status_map=status(transport_reserve_ok and hard_ok,communication_ok,relay_ok,component_ok); status_map['medical_hard_ok']=medical_ok; status_map['first_batch_hard_ok']=first_ok; status_map['hard_deadline_ok']=hard_ok; status_map['soft_lateness_s']=sum(max(0.,x['delivery_time_s']-x['deadline_s']) for x in dl if not x['hard']); status_map['timeline']=tl; status_map['deadline_ledger']=dl; status_map['transport_energy_ledger']=en
    return status_map
