...
chunks=[args[i:i+10] for i in range(0,len(args),10)]
def run(c):
 p=subprocess.run([sys.executable,str(ROOT/'src/q3_edge_batch_worker.py'),json.dumps(c)],cwd=ROOT,text=True,capture_output=True,timeout=500)
...
pd.DataFrame(rows).sort_values(['from_node','to_node']).to_csv(RES/'q3_full_edge_graph.csv',index=False,encoding='utf-8-sig')
