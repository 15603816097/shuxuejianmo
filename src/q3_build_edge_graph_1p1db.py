...
def main():
    ...
    pd.DataFrame(rows).to_csv(RES/'q3_full_edge_graph_1p1db.csv',index=False,encoding='utf-8-sig'); print(json.dumps({'records':len(rows),'runtime_seconds':time.time()-t0},ensure_ascii=False))
