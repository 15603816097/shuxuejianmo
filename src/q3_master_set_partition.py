...
def main():
    c=pd.read_csv(RES/'q3_candidate_pool.csv'); boxes=[]
    ...
    if c.empty or missing:
        (RES/'q3_master_summary.json').write_text(json.dumps({'master_status':'NO_COVERAGE','covered_boxes':len(set(boxes)),'missing_boxes':missing,'ROUTE_LEVEL_BEST_FOUND_K':None},ensure_ascii=False,indent=2),encoding='utf-8'); pd.DataFrame({'box_id':missing,'reason':'CANDIDATE_POOL_UNCOVERED'}).to_csv(RES/'q3_master_uncovered_boxes.csv',index=False,encoding='utf-8-sig'); return
    ...
    chosen=c.iloc[sel].copy(); chosen.to_csv(RES/'q3_master_selected_routes.csv',index=False,encoding='utf-8-sig')
    rows=[]
    for _,r in chosen.iterrows():
        for b in ast.literal_eval(r.box_ids): rows.append({'candidate_id':r.candidate_id,'box_id':b,'route':r.route})
    pd.DataFrame(rows).to_csv(RES/'q3_master_selected_boxes.csv',index=False,encoding='utf-8-sig')
    ...
    (RES/'q3_master_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
