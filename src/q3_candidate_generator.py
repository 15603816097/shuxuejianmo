...
def main():
 ...
 data=pipe.load_inputs(); graph=pd.read_csv(RES/'q3_full_edge_graph.csv'); typ='C'; feasible={(str(r.from_node),str(r.to_node)):bool(r.communication_possible) for _,r in graph[graph.drone_type==typ].iterrows()}; services=[f'S{i:03d}' for i in range(1,16)]
 ...
 pd.DataFrame(sk).to_csv(RES/'q3_route_skeletons.csv',index=False,encoding='utf-8-sig')
 ...
 out=pd.DataFrame(records).drop_duplicates(subset=['route','box_ids','drone_type']); out.to_csv(RES/'q3_candidate_pool_v2.csv',index=False,encoding='utf-8-sig'); all_boxes=sorted(bm); counts={b:0 for b in all_boxes}
 for z in out.box_ids:
  for b in json.loads(z): counts[b]+=1
 pd.DataFrame([{'box_id':b,'candidate_count':n,'covered':n>0,'reason':'HAS_CANDIDATE' if n else 'NO_PHYSICALLY_VALID_GENERATED_CANDIDATE'} for b,n in counts.items()]).to_csv(RES/'q3_candidate_coverage_v2.csv',index=False,encoding='utf-8-sig')
 summary={'candidate_count':len(out),'single':sum('>' not in x for x in out.visit_order),'double':sum(x.count('>')==1 for x in out.visit_order),'triple':sum(x.count('>')==2 for x in out.visit_order),'route_skeleton_count':len(skeletons),'covered_boxes':int(sum(n>0 for n in counts.values())),'uncovered_boxes':int(sum(n==0 for n in counts.values())),'min_candidates_per_box':int(min(counts.values())),'median_candidates_per_box':float(pd.Series(list(counts.values())).median()),'max_candidates_per_box':int(max(counts.values())),'frozen_sha256':SHA,'delta_backhaul_db':DELTA}; (RES/'q3_candidate_pool_v2_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
 diag=[]
 singleton_counts={}
 ...
 pd.DataFrame(diag).to_csv(RES/'q3_uncovered_box_diagnostic.csv',index=False,encoding='utf-8-sig'); print(json.dumps(summary,ensure_ascii=False))
