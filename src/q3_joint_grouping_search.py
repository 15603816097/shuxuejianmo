...
def solve(max_routes=24,time_limit=240):
    ...
    # objective: route count first, then relay-window span, then makespan and energy proxy
    ...
    m.Minimize(route_count*10**9 + span*10**4 + joint*10 + sum(energy_terms))

    solver=cp_model.CpSolver(); solver.parameters.max_time_in_seconds=time_limit; solver.parameters.num_search_workers=8
    st=solver.Solve(m)
    rec={"status":solver.StatusName(st),"candidate_count":len(cands),"max_routes":max_routes}
    selected=[]
    if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
        for i,c in enumerate(cands):
            if solver.Value(x[i]):
                ...
                selected.append(r)
        rec.update({"selected_routes":len(selected),"joint_completion_s":solver.Value(joint)/S,
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--max-routes",type=int,default=24); ap.add_argument("--time-limit",type=int,default=240)
    ap.add_argument("--out",default="results/q3_joint_grouping")
    a=ap.parse_args(); data,cands,sel,rec=solve(a.max_routes,a.time_limit)
    out=Path(a.out); out.mkdir(parents=True,exist_ok=True)
    # candidate audit
    pd.DataFrame([{**c,"box_ids":json.dumps(c["box_ids"],ensure_ascii=False),"relay_blocks":json.dumps(c["relay_blocks"])} for c in cands]).to_csv(out/"candidate_pool.csv",index=False,encoding="utf-8-sig")
    if sel:
        rows=[]
        for r in sel:
            rows.append({
