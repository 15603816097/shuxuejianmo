...
def build_model(data,tasks,cert,R,objective="zero",fixed=None,violation_cap=None):
    n=len(tasks); boxes={r["货箱编号"]:r for r in data["boxes"]}; starts=[]; box_task=[]; deadlines=[]; hard_required=[]
    for i,r in tasks.iterrows():
        ...
        for bid in ids:
            ...
            deadlines.append(min(vals) if vals else float(b.get("期望送达时间（s）", M)))
            hard_required.append(bool(vals))
    # variable registry
    names=[]; lb=[]; ub=[]; integ=[]; c=[]
    ...
    for k in range(len(deadlines)):
        # Medical and first-batch boxes are contractual hard constraints.
        # Their tardy variables remain fixed at zero so no objective or cap
        # can relax the physical deadline.  Only ordinary cargo is scored as
        # soft timeliness downstream.
        tard.append(var(f"tardy_box_{k}",0,0 if hard_required[k] else (0 if objective=="zero" and violation_cap is None else 1),1,1 if objective=="boxes" else 0))
    ...
    for i,r in tasks.iterrows():
        ...
        for _ in ids:
            if hard_required[k]:
                # Official hard deadline: delivery at the service area.
                add({sidx[i]:1},-np.inf,deadlines[k]-do)
            else:
                # Ordinary cargo keeps a soft lateness indicator.
