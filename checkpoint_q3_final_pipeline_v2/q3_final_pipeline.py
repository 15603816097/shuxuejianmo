...
from q3_semantics_core import (build_authoritative_timeline, deadline_ledger,
    ...
    component_ledger, xyz, link_obstructed, TOL)
        ...
        for off in (50.,100.,150.,200.,250.,300.):
            ...
            for q in block['items']:
                for ep,eh in ((q['a'],q['ha']),(q['b'],q['hb'])):
                    dist=((xyz(data,ep,eh)-R)**2).sum()**.5; access_obs=obs if link_obstructed(data,ep,p,eh,h) else 0.0; access.append(ta-(32.45+20*math.log10(freq)+20*math.log10(max(dist/1000,1e-12))+access_obs))
            back_dist=((R-G)**2).sum()**.5; back_obs=obs if link_obstructed(data,p,c,h,float(c['海拔（m）'])+float(data['comm'][('固定网关 G01','天线离地高度（m）')])) else 0.0; back=tb-(32.45+20*math.log10(freq)+20*math.log10(max(back_dist/1000,1e-12))+back_obs); z={'x':p['经度（°）'],'y':p['纬度（°）'],'altitude':h,'terrain_elevation_m':max(vals),'agl_m':off,'dem_query_ok':True,'access_margin':min(access),'backhaul_margin':back,'joint_margin':min(min(access),back),'candidate_source':'trajectory_3d_endpoints_plus_local_xy_offsets','dem_legal':bool(off>=0 and off<=data['relay']['max_height_m']+TOL),'height_legal':bool(off>=0 and off<=data['relay']['max_height_m']+TOL)}
